...
export interface BrowserIdentity {
  ...
  operationId?: string;
  enrolled?: boolean;
  pendingRedemption?: Record<string, unknown>;
}
...
async function database() {
  return new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(new Error("Browser credential storage unavailable."));
  });
}
export async function readIdentity(): Promise<BrowserIdentity | undefined> {
  ...
  try {
    return await new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => resolve(request.result);
      request.onerror = () =>
        reject(new Error("Cannot read browser identity."));
    });
  } finally {
    db.close();
  }
...
export async function saveIdentity(identity: BrowserIdentity) {
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      tx.onerror = tx.onabort = () =>
        reject(
          new Error(
            "Cannot durably save browser identity. Recovery may be required.",
          ),
        );
    });
  } finally {
  ...
  if (
    saved?.token !== identity.token ||
    saved?.operationId !== identity.operationId ||
    saved?.deviceId !== identity.deviceId
  )
    throw new Error("Browser credential readback failed.");
}
...
export async function browserIdentity(
  ...
  if (!identity) {
    ...
    identity = {
      ...
      publicKey: hex(raw),
      deviceId: `dev_${hex(await crypto.subtle.digest("SHA-256", raw))}`,
      origin: location.origin,
    };
    await saveIdentity(identity);
  }
  if (identity.origin !== location.origin)
    throw new Error(
      "Browser identity belongs to another origin. Explicit setup required.",
    );
  if (identity.token && !recover) {
    await maintainIdentity(identity); // Validate and finish a saved ACK before using voice.
    return identity;
  }
  if (identity.enrolled && !recover && identity.pendingRedemption) {
    const c = identity.pendingRedemption;
    const signature = hex(
      await crypto.subtle.sign(
        "Ed25519",
        identity.privateKey,
        new TextEncoder().encode(transcript("status", c)),
      ),
    );
    const status = await ownerPost("/api/enrollment/v1/status", {
      request_id: c.request_id,
      signature,
    });
    if (status.status === "approved") return redeemIdentity(identity, c);
  }
  if (identity.enrolled && !recover)
    throw new Error(
      "Browser access retired or enrollment outcome uncertain. Choose Recover browser identity.",
    );
  if (recover) {
    const operation = {
    ...
    const rows = JSON.parse(sessionStorage.getItem("vauxr-operations") || "[]");
    sessionStorage.setItem(
      "vauxr-operations",
      JSON.stringify([...rows, operation]),
    );
    window.dispatchEvent(new Event("lifecycle-operation"));
    await ownerPost("/api/lifecycle/v1/recover", {
  ...
  const c = await ownerPost("/api/enrollment/v1/request", {
    kind: "browser",
  ...
  if (
    ...
    !/^[a-f0-9]{32}$/.test(c.owner_generation) ||
  ...
  const sign = async (action: string) =>
    hex(
      await crypto.subtle.sign(
        "Ed25519",
        identity.privateKey,
        new TextEncoder().encode(transcript(action, c)),
      ),
    );
  const proof = await ownerPost("/api/enrollment/v1/prove", {
    request_id: c.request_id,
    signature: await sign("prove"),
  });
  await ownerPost("/api/enrollment/v1/initiate", {
    request_id: c.request_id,
    code: proof.code,
  });
  await ownerPost("/api/enrollment/v1/approve", {
    request_id: c.request_id,
    code: proof.code,
  });
  // Retain uncertainty before the one-shot redemption. Never silently create another key.
  identity.enrolled = true;
  identity.pendingRedemption = c;
  await saveIdentity(identity);
  return redeemIdentity(identity, c);
}
async function redeemIdentity(
  ...
  const signature = hex(
    await crypto.subtle.sign(
      "Ed25519",
      identity.privateKey,
      new TextEncoder().encode(transcript("redeem", c)),
    ),
  );
  const result = await ownerPost("/api/enrollment/v1/redeem", {
    request_id: c.request_id,
    signature,
  });
  if (
    result.device_id !== identity.deviceId ||
    !/^vx_dev_[A-Za-z0-9_-]{43}$/.test(result.device_token)
  )
    throw new Error(
      "Invalid browser credential response. Recover browser identity.",
    );
  identity.pendingRedemption = undefined;
...
async function acknowledge(identity: BrowserIdentity) {
  await clientPost("ack", identity.token!, {
...
export async function maintainIdentity(identity: BrowserIdentity) {
  ...
  if (operation.state === "pending" || operation.state === "queued") {
    const result = await clientPost("deliver", identity.token!, {
      operation_id: operation.operation_id,
    });
    if (
      result.operation_id !== operation.operation_id ||
      result.role !== "device" ||
      result.subject !== identity.deviceId ||
      !/^vx_dev_[A-Za-z0-9_-]{43}$/.test(result.credential)
    )
      throw new Error(
        "Invalid credential delivery. Explicit recovery required.",
      );
    const replacement = {
      ...identity,
    ...
    throw new Error(
      "Credential delivery was not acknowledged. Recover browser identity.",
    );
  }
}
export async function retireBrowser() {
  ...
  const retire = async () => {
    ...
    await ownerPost("/api/lifecycle/v1/revoke", {
      operation_id: id,
      role: "device",
      subject: identity.deviceId,
    });
  ...
  channel.close();
  if (navigator.locks)
    await navigator.locks.request("vauxr-browser-voice", retire);
  else await retire();
}
