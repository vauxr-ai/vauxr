"""Legacy agent hashes remain routing data, never authorization authority.
...
def _write_node_style_agents(data_dir: Path, raw_token: str, agent_id: str = "node-ch") -> None:
    """Write agents.json the way the old Node/bcryptjs server would.
    ...
    (data_dir / "agents.json").write_text(json.dumps(payload, indent=2))
...
async def test_oversize_bearer_token_returns_401_not_500(client: TestClient) -> None:
    ...
    oversize = "x" * 200
    res = await client.get("/api/agents", headers=_bearer(oversize))
...
async def test_oversize_bearer_token_on_rotate_returns_401_not_500(
    ...
    res = await client.post(
        f"/api/agents/{agent.id}/rotate", headers=_bearer(oversize)
    )
    assert res.status == 401, await res.text()
...
async def test_node_migrated_agent_requires_reenrollment(
    client: TestClient, tmp_path: Path
) -> None:
    """A agent created by the old Node/bcryptjs server (with `$2a$`
    hash) must not authenticate after the Python rewrite reads
    agents.json at startup."""
    raw_token = "vx_ag_" + "0123456789abcdef" * 4  # canonical 70-byte token
...
async def test_legacy_device_token_cannot_rotate(
    ...
    """`POST /api/agents/{id}/rotate` against a Node-migrated agent,
